...
public class ValidateBST {
  ...
  public static Tuple<Integer, Integer, Boolean> checkBST(TreeNode<Integer> root) {
    ...
    return new Tuple<>(
        left == null ? root.getValue() : left.getA(),
        right == null ? root.getValue() : right.getB(),
        validBST);
  }
  ...
  public static boolean isBSTPreOrder(TreeNode<Integer> root, Integer min, Integer max) {
    ...
    if (isBSTPreOrder(root.getLeft(), min, root.getValue()) && isBSTPreOrder(root.getRight(), root.getValue(), max)) {
      return true;
    }
