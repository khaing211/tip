...
public class SummationOfPrimes {
  ...
  public long sumPrimesLessThan(long n) {
    if (n <= queryPrimeMaxCeiling) {
      ...
      if (index == -1)
        return 0;
      if (index == knownPrimes.size() || knownPrimes.get(index) > n)
        index -= 1;
      ...
      for (queryPrimeMaxCeiling += 2; queryPrimeMaxCeiling <= n; queryPrimeMaxCeiling += 2) {
        if (isPrime(queryPrimeMaxCeiling)) {
          primesToSum.put(queryPrimeMaxCeiling,
              primesToSum.get(knownPrimes.get(knownPrimes.size() - 1)) + queryPrimeMaxCeiling);
