 ...
 *
 * Input Format First line contains T that denotes the number of test cases. First line of each test
 * case will contain two integers N & K. Second line of each test case will contain a N digit
 * integer.
 *
  ...
  public static int compute(int[] series, int k) {
    ...
    for (int i = 0; i < series.length - k; i++) {
      final int currentProduct =
          IntStream.range(i, i + k).map(it -> series[it]).reduce(1, (int l, int r) -> l * r);
