 ...
 *
 * For instance, the sequence 5, 7, 9, 11, 13, 15 … is an arithmetic progression with common
 * difference of 2.
 */
public interface ArithmeticProgression {

  /**
   * Given a arithmetic progression with a missing term Find the missing term
   *
   * Constraint: + sequence.size() >= 3 + term can be positive or negative + term can be between
   * [-10^6, 10^6]
   */
  public static long findMissingTerms(List<Long> sequence) {
    ...
    final long arithmeticSum =
        (sequence.get(0) + sequence.get(lastIndex)) * (sequence.size() + 1) / 2;
  ...
  public static boolean isArithmeticSequence(List<Long> sequence) {
    ...
    while (iterator.hasNext()) {
      ...
      if (currentTerm - previousTerm != diffConstant)
        return false;
