...
public class ChainingHashMap<K, V> extends AbstractMap<K, V> {
  ...
  public static void main(String[] args) {
    ...
    for (int step = 0; step < 1000; step++) {
      ...
      for (int i = 0; i < 1000; i++) {
        ...
        check(m1, m2, op < methods0.length ? methods0[op] : methods1[op - methods0.length],
            op < methods0.length ? null : arg);
  ...
  static void check(Object obj1, Object obj2, String methodName, Integer arg) {
    ...
    if (!((result1 instanceof Exception && result2 instanceof Exception)
        || "next".equals(methodName) || Objects.deepEquals(result1, result2)))
      throw new RuntimeException(result1 + " " + result2);
  }

  static Object invoke(Object obj, String methodName, Integer arg) {
    try {
      Method method = arg != null ? obj.getClass().getMethod(methodName, Object.class)
