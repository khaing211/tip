...
public class ContractionHierarchies {
  ...
  public static class LayerGraph implements Cloneable {
    ...
    public Object clone() {
      ...
      g.edges = edges;
      int[][] arrays1 =
          {levels, firstEdge, secondEdge, len, u, v, tail[0], tail[1], prev[0], prev[1]};
      int[][] arrays2 = {g.levels, g.firstEdge, g.secondEdge, g.len, g.u, g.v, g.tail[0], g.tail[1],
          g.prev[0], g.prev[1]};
  ...
  private static List<Integer> findWitness(LayerGraph g, int s, int forbidden, int[] prio,
      boolean[] targets, int targetCount, int[] hops, int upperBound) {
    hops[s] = 0;
  ...
  private static ShortcutsInfo addShortcuts(LayerGraph g, int v, boolean realRun, int[] prio,
      boolean[] targets, int[] hops, int[] degree, int[] originalEdges) {
    int shortcuts = 0;
    ...
    for (int uv = g.tail[1][v]; uv != -1; uv = g.prev[1][uv]) {
      ...
      List<Integer> visited =
          findWitness(g, u, v, prio, targets, targetCount, hops, g.len[uv] + maxLenVW);
  ...
  private static int calcPriority(LayerGraph g, int v, int[] prio, boolean[] targets, int[] hops,
      int[] degree, int[] originalEdges) {
    ShortcutsInfo shortcutsInfo =
        addShortcuts(g, v, false, prio, targets, hops, degree, originalEdges);
  ...
  public static LayerGraph preprocess(LayerGraph origGraph) {
    ...
    for (int v = 0; v < g.nodes; v++)
      priorities
          .add(((long) calcPriority(g, v, prio, targets, hops, degree, originalEdges) << 32) | v);
    Arrays.fill(g.levels, Integer.MAX_VALUE);

    for (int i = 0; i < g.nodes - 2; i++) {
      while (g.levels[priorities.peek().intValue()] != Integer.MAX_VALUE)
        priorities.remove();
      ...
      for (int edge = g.tail[0][v]; edge != -1; edge = g.prev[0][edge]) {
        ...
        if (g.levels[w] == Integer.MAX_VALUE)
          priorities.add(
              ((long) calcPriority(g, w, prio, targets, hops, degree, originalEdges) << 32) | w);
      }
      for (int edge = g.tail[1][v]; edge != -1; edge = g.prev[1][edge]) {
        int u = g.u[edge];
        if (g.levels[u] == Integer.MAX_VALUE)
          priorities.add(
              ((long) calcPriority(g, u, prio, targets, hops, degree, originalEdges) << 32) | u);
      }
    }
  ...
  public static PathInfo shortestPath(LayerGraph g, int s, int t) {
    ...
    PriorityQueue<Long>[] q =
        new PriorityQueue[] {new PriorityQueue<Long>(), new PriorityQueue<Long>()};
    q[0].add((long) s);
    q[1].add((long) t);
    int res = Integer.MAX_VALUE - 1;
    int top = -1;
    m1: for (int dir = 0;; dir = !q[1 - dir].isEmpty() ? 1 - dir : dir) {
      if (res <= Math.min(q[0].isEmpty() ? Integer.MAX_VALUE : (int) (q[0].peek() >>> 32),
          q[1].isEmpty() ? Integer.MAX_VALUE : (int) (q[1].peek() >>> 32)))
        break;
  ...
  public static void main(String[] args) {
    ...
    for (int step = 0; step < 100; step++) {
      ...
      if (origGraph.edges > E)
        throw new RuntimeException(E + " " + origGraph.edges);
      ...
      System.out.println(
          "edges = " + origGraph.edges + " shortcuts = " + shortcuts + " nodes = " + g.nodes);
