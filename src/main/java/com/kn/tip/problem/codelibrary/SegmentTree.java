...
public class SegmentTree {
  ...
  // This example implements Add/Max operations. Operations like Add/Sum, Set/Max can also be
  // implemented.
  ...
  void pushDelta(int root, int left, int right) {
    value[root] =
        joinValueWithDelta(value[root], deltaEffectOnSegment(delta[root], right - left + 1));
  ...
  int query(int from, int to, int root, int left, int right) {
    ...
    int mid = (left + right) >> 1;
    if (from <= mid && to > mid)
      return queryOperation(query(from, Math.min(to, mid), root * 2 + 1, left, mid),
  ...
  void modify(int from, int to, int delta, int root, int left, int right) {
    ...
    value[root] = queryOperation(
        joinValueWithDelta(value[2 * root + 1],
            deltaEffectOnSegment(this.delta[2 * root + 1], mid - left + 1)),
        joinValueWithDelta(value[2 * root + 2],
            deltaEffectOnSegment(this.delta[2 * root + 2], right - mid)));
  }
