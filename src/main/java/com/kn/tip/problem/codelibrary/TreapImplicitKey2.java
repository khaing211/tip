...
public class TreapImplicitKey2 {
  ...
  static void applyDelta(Treap root, int delta) {
    ...
    root.subTreeValue =
        joinValueWithDelta(root.subTreeValue, deltaEffectOnSegment(delta, root.size));
  }
  ...
  public static class Treap {
    int nodeValue;
    int subTreeValue;
    int delta; // delta affects left.nodeValue, right.nodeValue, left.subTreeValue,
               // right.subTreeValue, left.delta and right.delta
    int size;
    long prio;
    Treap left;
    Treap right;
    ...
    void update() {
      subTreeValue =
          queryOperation(queryOperation(getSubTreeValue(left), nodeValue), getSubTreeValue(right));
