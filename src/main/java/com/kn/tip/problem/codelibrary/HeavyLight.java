...
public class HeavyLight {
  ...
  public int query(int u, int v) {
    ...
    processPath(u, v,
        (a, b) -> res.set(segmentTree.queryOperation(res.get(), segmentTree.query(a, b))));
    return res.get();
  }
  ...
  static class SegmentTree {
    ...
    // This example implements Add/Sum operations. Operations like Add/Max, Set/Max can also be
    // implemented.
    ...
    public int query(int from, int to) {
      ...
      for (int len = 1; from <= to; from = (from + 1) >> 1, to = (to - 1) >> 1, len <<= 1) {
        if ((from & 1) != 0) {
          res = found ? queryOperation(res, joinNodeValueWithDelta(from, len))
              : joinNodeValueWithDelta(from, len);
          found = true;
        }
        if ((to & 1) == 0) {
          res = found ? queryOperation(res, joinNodeValueWithDelta(to, len))
    ...
    public void modify(int from, int to, int delta) {
      ...
      for (int i = a, len = 1; i > 1; i >>= 1, len <<= 1) {
        value[i >> 1] =
            queryOperation(joinNodeValueWithDelta(i, len), joinNodeValueWithDelta(i ^ 1, len));
      }
  ...
  static List<Integer>[] getRandomTree(int n, Random rnd) {
    ...
    int[] p = new int[n];
    for (int i = 0, j; i < n; j = rnd.nextInt(i + 1), p[i] = p[j], p[j] = i, i++); // random
                                                                                   // permutation
    for (int i = 1; i < n; i++) {
