...
public class CircleOperations {
  ...
  static Point[] intersection(double a, double b, double c, double CX, double CY, double R,
      boolean swap) {
    // ax+by+c=0
    ...
    double x2 = (-c - b * y2) / a;
    return swap
        ? d > EPS ? new Point[] {new Point(y1, x1), new Point(y2, x2)}
  ...
  static void tangents(Point center2, double r1, double r2, List<Line> lines) {
    ...
    d = Math.sqrt(d < 0 ? 0 : d);
    lines.add(
        new Line((center2.x * r + center2.y * d) / z, (center2.y * r - center2.x * d) / z, r1));
  }
  ...
  public static void main(String[] args) {
    ...
    for (int step = 0; step < 100_000; step++) {
      ...
      Point[] p1 = circleLineIntersection(new Circle(x, y, r),
          new Line(new Point(x1, y1), new Point(x2, y2)));
      Point[] p2 = circleLineIntersection2(new Circle(x, y, r),
          new Line(new Point(x1, y1), new Point(x2, y2)));

      if (p1.length != p2.length || p1.length == 1 && !eq(p1[0], p2[0]) || p1.length == 2
          && !(eq(p1[0], p2[0]) && eq(p1[1], p2[1]) || eq(p1[0], p2[1]) && eq(p1[1], p2[0])))
