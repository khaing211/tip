...
public class ConvexCut {
  ...
  public static Point[] convexCut(Point[] poly, Point p1, Point p2) {
    ...
    for (int i = 0, j = n - 1; i < n; j = i++) {
      ...
      if (d1 >= 0)
        res.add(poly[j]);
      if (d1 * d2 < 0)
        res.add(getLinesIntersection(p1.x, p1.y, p2.x, p2.y, poly[j].x, poly[j].y, poly[i].x,
            poly[i].y));
    }
  ...
  static Point getLinesIntersection(double x1, double y1, double x2, double y2, double x3,
