...
public class PointInPolygon {
  ...
  public static int pointInPolygon(int qx, int qy, int[] x, int[] y) {
    ...
    for (int i = 0, j = n - 1; i < n; j = i++) {
      if (y[i] == qy
          && (x[i] == qx || y[j] == qy && (x[i] <= qx || x[j] <= qx) && (x[i] >= qx || x[j] >= qx)))
        return 0; // boundary
      if ((y[i] > qy) != (y[j] > qy)) {
        long det =
            ((long) x[i] - qx) * ((long) y[j] - qy) - ((long) x[j] - qx) * ((long) y[i] - qy);
        if (det == 0)
          return 0; // boundary
        if ((det > 0) != (y[j] > y[i]))
          ++cnt;
  ...
  static boolean isCrossOrTouchIntersect(long x1, long y1, long x2, long y2, long x3, long y3,
      long x4, long y4) {
  ...
  static boolean isCrossIntersect(long x1, long y1, long x2, long y2, long x3, long y3, long x4,
      long y4) {
    long z1 = (x2 - x1) * (y3 - y1) - (y2 - y1) * (x3 - x1);
    long z2 = (x2 - x1) * (y4 - y1) - (y2 - y1) * (x4 - x1);
    long z3 = (x4 - x3) * (y1 - y3) - (y4 - y3) * (x1 - x3);
