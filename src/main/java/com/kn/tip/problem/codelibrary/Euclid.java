...
public class Euclid {
  ...
  public static BigInteger garnerRestore(int[] a, int[] p) {
    ...
    for (int i = 0; i < x.length; ++i)
      for (int j = 0; j < i; ++j)
        x[i] = mod(BigInteger.valueOf(p[j]).modInverse(BigInteger.valueOf(p[i])).longValue()
            * (x[i] - x[j]), p[i]);
  ...
  public static void main(String[] args) {
    ...
    for (int steps = 0; steps < 10000; steps++) {
      ...
      if (gcd1 != gcd || gcd2 != gcd || !Arrays.equals(euclid1, euclid2) || euclid1[0] != gcd
          || inv1 != inv || inv2 != inv) {
    ...
    long b = 9;
    long[] res = euclid(a, b);
    System.out.println(res[1] + " * (" + a + ") " + " + " + res[2] + " * (" + b + ") = gcd(" + a
        + "," + b + ") = " + res[0]);
