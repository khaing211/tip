...
import com.kn.tip.datastructure.ConsList;
import com.kn.tip.datastructure.TreeNode;
import com.kn.tip.utils.MathUtils;
 ...
 *
 * 1. longest path between two leaves go through root node of tree or subtree. 2. height is the
 * longest path to a leave of tree / subtree. 3. height function be cached, otherwise the
 * performance like fibonanci computation
 */
public class TreeDiameter {
  ...
  public static <T> int diameter(TreeNode<T> root, Map<TreeNode<T>, Integer> heightCache) {
    ...
    return MathUtils.max(lheight + rheight + 1, diameter(root.getLeft(), heightCache),
        diameter(root.getRight(), heightCache));
  }
  ...
  public static <T> int height(TreeNode<T> root, Map<TreeNode<T>, Integer> heightCache) {
    ...
    return 1 + Math.max(
        heightCache.get(root.getLeft()) != null ? heightCache.get(root.getLeft())
            : height(root.getLeft(), heightCache),
        heightCache.get(root.getRight()) != null ? heightCache.get(root.getRight())
            : height(root.getRight(), heightCache));
  }
  ...
  public static <T> List<TreeNode<T>> diamaterPath(TreeNode<T> root) {
    ...
    // normally, this could implement as in OO manner by decorator pattern
    final Map<TreeNode<T>, ConsList<TreeNode<T>>> heightCache =
        new HashMap<TreeNode<T>, ConsList<TreeNode<T>>>();

    return null;
  }

  public static <T> ConsList<T> diameterPath(TreeNode<T> root,
      Map<TreeNode<T>, ConsList<TreeNode<T>>> heightCache) {
