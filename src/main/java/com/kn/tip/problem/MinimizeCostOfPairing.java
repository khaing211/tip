...
import com.kn.tip.datastructure.IntPair;

/**
 * Let (x,y) be the coordinates of a student's house on a 2D plane. There are 2N students and we
 * wanted to pair them into N groups. Let d_i be the distance between the house of 2 students in
 * group i. Form N groups such that cost = (sum from i to N) d_i is minimized. Output the minimized
 * cost
 *
 * Constraint 1 <= N <= 8 0 <= x,y <= 1000
 *
 */
  ...
  static void computeDistance(IntPair[] coordinates, double[][] distance) {
    for (int i = 0; i < coordinates.length; i++) {
      for (int j = i + 1; j < coordinates.length; j++) {
        distance[i][j] =
            distance[j][i] = Math.hypot(coordinates[i].getFirst() - coordinates[j].getFirst(),
                coordinates[i].getSecond() - coordinates[j].getSecond());
      }
    }
  }

  static double computeDistance(int currentSet, int targetSet, int N, double[][] distance,
      double[] minDistance) {
    ...
    for (p2 = p1 + 1; p2 < N; p2++) {
      ...
      if ((currentSet & mask) == 0) {
        answer =
            Math.min(answer, distance[p1][p2] + computeDistance(currentSet | (1 << p1) | (1 << p2),
                targetSet, N, distance, minDistance));
      }
    }
