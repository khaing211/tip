 ...
 * price of Wooden Orange Toothpicks Inc. (WOT) will be for the next N days.
 *
 * Each day, you can either buy one share of WOT, sell any number of shares of WOT that you own, or
 * not make any transaction at all. What is the maximum profit you can obtain with an optimum
 * trading strategy?
 *
 * Input
 *
 ...
 *
 * The first line of each test case contains a number N. The next line contains N integers, denoting
 * the predicted price of WOT shares for the next N days.
 *
 * Output
 *
 * Output T lines, containing the maximum profit which can be obtained for the corresponding test
 * case.
 *
 * Constraints
 *
...
public interface StockMaximize {
  public static long computeProfit(int[] prices) {
    ...
    // supposedly, the only reason is you don't sell all (i.e. sell less than all) at price[i] on
    // day i
    ...
    // profits[i] = max( profits[j] + sum_{j <= k < i} max(prices[i] - prices[k], 0) ) for 0 <= j <
    // i
    for (int i = 1; i < prices.length; i++) {
      ...
      /*
       * for (int j = 0; j < i; j++) { long profitEarn = 0;
       * 
       * for (int k = j; k < i; k++) { profitEarn += Math.max(prices[i] - prices[k], 0); }
       * 
