...
public interface DijkstraShortestPath {
  ...
  static int distance(GridWeightedGraph graph, int src, int dest) {
    ...
    PriorityQueue<Node> queue =
        new PriorityQueue<Node>(Comparator.comparingInt(it -> it.getDistTo()));
    // add() instead of push()
    ...
    while (!queue.isEmpty()) {
      ...
      if (currentNode.getNode() == dest)
        return currentNode.distTo;
      ...
      for (int i = 0; i < graph.getNumNode(); i++) {
        if (graph.isNeighbor(currentNode.getNode(), i) && !visited[i]) {
          queue.add(
              new Node(i, currentNode.getDistTo() + graph.getWeight(currentNode.getNode(), i)));
        }
      }
    }
