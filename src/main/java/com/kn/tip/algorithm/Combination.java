...
public interface Combination {
   ...
   * 
   * Recursively generate combination d row = d c row = c+{}, c+d = c, cd b row = b+{}, b+d, b+c,
   * b+cd = b, bd, bc, bcd c row = a+{}, a+d, a+c, a+cd, a+b, a+bc, a+bd, a+bcd = a, ad, ac, acd,
   * ab, abc, abd, abcd
   */
  ...
  /**
   * Given an array of size n, return all combination of 2 elements of the array
   * 
   * @return List of indices pair
   * 
   *         The key to generate is nested loop inner loop start index is equal to outer loop
   *         current
   * 
  ...
  /**
   * Given an array of size n, return all combination of k elements of the array
   * 
   * @return List of k-tuples indices
   * 
   *         The idea is same as choose2, but since number of loop is dynamic i.e. k
   * 
   *         we maintain an indicies array i.e. set below where set[0] = outermost loop indices
   * 
   *         when we call next(set, n, k), it will generate the next in-order set using same logic
   *         where inner loop start index is equal to outer loop current i.e. set[i+1] = set[i]+1
   */
  static List<int[]> choose(final int n, final int k) {
    ...
    do {
      sets.add(Arrays.copyOf(indices, k));
    } while (next(indices, n, k));

    return sets;
  }

  static boolean next(int[] indices, int n, int k) {
    int i = k - 1;
    while (i >= 0) {
