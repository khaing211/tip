...
public interface PrimMinimumSpanningTree {
  static class Edge {
    ...
    public String toString() {
      return "Edge [dist=" + dist + ", toNode=" + toNode + ", fromNode=" + fromNode + "]";
    }
  }
  ...
  static Edge[] compute(GridWeightedGraph graph) {
    ...
    while (!queue.isEmpty()) {
      ...
      if (visited[currentEdge.getToNode()])
        continue;
      ...
      for (int i = 0; i < N; i++) {
        if (graph.isNeighbor(currentEdge.getToNode(), i) && !visited[i]) {
          queue.add(
              new Edge(currentEdge.getToNode(), i, graph.getWeight(currentEdge.getToNode(), i)));
        }
      }
    }
