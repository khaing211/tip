...
/**
 * The <tt>FordFulkerson</tt> class represents a data type for computing a <em>maximum st-flow</em>
 * and <em>minimum st-cut</em> in a flow network.
 * <p>
 * This implementation uses the <em>Ford-Fulkerson</em> algorithm with the
 * <em>shortest augmenting path</em> heuristic. The constructor takes time proportional to
 * <em>E V</em> (<em>E</em> + <em>V</em>) in the worst case and extra space (not including the
 * network) proportional to <em>V</em>, where <em>V</em> is the number of vertices and <em>E</em> is
 * the number of edges. In practice, the algorithm will run much faster. Afterwards, the
 * <tt>inCut()</tt> and <tt>value()</tt> methods take constant time.
 * <p>
 ...
 * 6.4</a> <i>Algorithms, 4th Edition</i> by Robert Sedgewick and Kevin Wayne.
 *
...
public class FordFulkerson {
  ...
  /**
   * Compute a maximum flow and minimum cut in the network <tt>G</tt> from vertex <tt>s</tt> to
   * vertex <tt>t</tt>.
   * 
   ...
   * 
   * @return <tt>true</tt> if vertex <tt>v</tt> is on the <tt>s</tt> side of the micut, and
   *         <tt>false</tt> if vertex <tt>v</tt> is on the <tt>t</tt> side.
