...
import com.kn.tip.utils.ListUtils;
 ...
 * 2. Parent index = (child index - 1) / 2 (integer division) 3. left child index = 2 * parent index
 * + 1 4. right child index = 2 * parent index + 2 5. find max/min is O(1) 6. delete max/min or
 * insert is O(log n) = heapify / sift up / sift down process 7. random access delete could be
 * implemented but it takes O(log n)
 */
public class Heap<T> {
  ...
  public Heap(List<T> list, Comparator<T> comparator) {
    if (!(list instanceof RandomAccess)) {
      throw new IllegalArgumentException("The list passed has to support random access.");
    }
